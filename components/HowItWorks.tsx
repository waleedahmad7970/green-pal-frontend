...
const steps = [
    {
        ...
        subtitle: "STEP 1",
        title: "Rent Battery",
        description: "Simply scan the QR code or tap your method of payment on the card reader to start your battery rental.",
    ...
    },
    {
        ...
        description: "Take the power bank with you. Our high-capacity batteries come with all cables built-in for ultimate convenience.",
    ...
    },
    {
        ...
        description: "Done charging? Just push the power bank into any empty slot at any Greenpal station globally to end your rental.",
    ...
    useEffect(() => {
        const ctx = gsap.context(() => {
            ...
            mm.add("(min-width: 768px)", () => {
                ...
                gsap.set(`.progress-${activeIndex}`, { scaleX: 1, scaleY: 0, transformOrigin: "top center" });
            ...
            mm.add("(max-width: 767px)", () => {
                ...
                gsap.set(`.progress-${activeIndex}`, { scaleY: 1, scaleX: 0, transformOrigin: "left center" });
                progressTween.current = gsap.to(`.progress-${activeIndex}`, {
                    scaleX: 1,
                    duration: AUTO_PLAY_DURATION,
                    ease: "none",
                    ...
                    {steps.map((step, i) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                */}
                                <div className="absolute top-0 left-0 w-full md:w-20 h-[72px] md:h-full border-b md:border-b-0 md:border-r line-rule bg-surface z-20 flex flex-row md:flex-col items-center justify-between p-5 md:py-8">
                                    ...
                                    <div className="flex items-center gap-4">
                                        <span className={`font-display text-xl md:text-2xl font-bold transition-colors duration-500 ${isActive ? 'text-signal' : 'text-muted group-hover:text-ink'}`}>
                                            0{step.id}
                                        </span>
                                        {/* Mobile Only: Show the title next to the number so it doesn't look empty */}
                                        <span className={`md:hidden font-display text-lg font-bold transition-colors duration-500 ${isActive ? 'text-ink' : 'text-muted group-hover:text-ink'}`}>
                                            {step.title}
                                        </span>
                                    </div>
                                    ...
                                    <div className="relative flex-1 w-full md:w-1 h-1 md:h-full ml-4 md:ml-0 md:mt-6 bg-ink/10 rounded-full overflow-hidden">
                                        <div className={`accordion-progress progress-${i} absolute top-0 left-0 w-full h-full bg-signal rounded-full`} />
                                    </div>
                                </div>
                                ...
                                */}
                                <div className={`absolute inset-0 pt-[72px] md:pt-0 md:pl-20 flex flex-col md:flex-row w-full md:w-[1000px] transition-opacity duration-500 z-10
