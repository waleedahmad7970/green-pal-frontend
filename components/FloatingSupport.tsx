...
export default function FloatingSupport() {
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      let botReply =
        "Thanks for your message! This is a placeholder response until the real API is connected.";

      const lowerInput = newUserMsg.text.toLowerCase();
      if (lowerInput.includes("pricing") || lowerInput.includes("cost")) {
        botReply =
          "Our hardware pricing is tiered based on volume. You can check the hardware catalog for specific numbers!";
        ...
        botReply =
          "Our blueprints break down exact capital layouts, revenue shares, and ROI for different venue types.";
      }
    ...
    <div className="fixed bottom-6 right-6 md:bottom-10 md:right-10 z-[100] flex flex-col items-end">
          ...
          >
            ...
            <div className="flex items-center justify-between p-5 border-b line-rule bg-surface/80 backdrop-blur-md shrink-0">
              <div className="flex items-center gap-3">
                ...
                {/* FORCED CSS VARIABLE INSTEAD OF RELYING ON TAILWIND */}
                <h3 className="font-display font-bold text-lg text-[rgb(var(--fg))]">
                  Greenpal Support
                </h3>
              </div>
              <button
                onClick={() => setIsOpen(false)}
                className="text-muted hover:text-[rgb(var(--fg))] transition-colors"
              >
                <svg
                  className="w-5 h-5"
                  fill="none"
                  ...
                  <div
                    className={`max-w-[80%] p-3.5 rounded-2xl shadow-sm ${
                      msg.role === "user"
                        ...
                        : "bg-card border line-rule text-[rgb(var(--fg))] rounded-bl-sm"
                    }`}
                  >
                    {msg.text}
                  </div>
            ...
            <form
              ...
              className="p-4 bg-card border-t line-rule shrink-0"
            >
              <div className="relative flex items-center">
                <input
                  ...
                  className="w-full bg-surface border line-rule rounded-full pl-4 pr-12 py-3 font-body text-sm text-[rgb(var(--fg))] placeholder:text-[rgb(var(--fg-muted))] focus:outline-none focus:border-signal transition-colors"
                />
                <button
                  type="submit"
      ...
      >
        ...
        >
          {/* Liquid hover fill */}
          <div
            className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] h-[300px] bg-signal rounded-full transition-transform duration-[600ms] ease-signature pointer-events-none ${!isOpen ? "scale-0 group-hover:scale-100" : "scale-0"}`}
          />
          ...
          <AnimatePresence>
            {isHovered && !isOpen && (
              <motion.div
                ...
                className="absolute left-6 whitespace-nowrap text-[rgb(var(--fg))] group-hover:text-ink font-display font-bold text-lg tracking-wide transition-colors duration-[600ms] pointer-events-none"
              >
                Let's Chat
              </motion.div>
            )}
          ...
          <div className="relative z-10 flex items-center justify-center w-16 h-16 shrink-0 text-[rgb(var(--fg))] group-hover:text-ink transition-colors duration-[600ms]">
            <motion.div
